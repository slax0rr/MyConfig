...
BEEP="`which paplay` /usr/share/sounds/remote-notif/chime.wav"	# loud!
#BEEP='/usr/bin/beep'
...
(
	...
	read network from msg; do
		...
		xmsg=`/bin/echo "${msg}" | /usr/bin/recode ascii..html`	# escape
		$NOTIFY -i gtk-dialog-info -t 300000 -- "${network}(${from})" "${msg//</[}";
		$BEEP
	done
) &
