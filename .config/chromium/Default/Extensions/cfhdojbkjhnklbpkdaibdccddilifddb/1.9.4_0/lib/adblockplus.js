...
require.scopes["elemHide"] = (function()
{
  ...
  var ElemHide = exports.ElemHide = {
    ...
    init: function()
    {
      var messageManager = Cc["@mozilla.org/parentprocessmessagemanager;1"].getService(Ci.nsIMessageListenerManager);
      var hitHandler = function(message)
      {
        var result = ElemHide.shouldHide(message.data);
        var target = message.target.QueryInterface(Ci.nsIMessageSender);
      ...
      {
        return messageManager.removeMessageListener("AdblockPlus:ElemHideHit", hitHandler);
      });
    ...
    {
      var filter = ElemHide.getFilterByKey(message.key);
      if (!filter || !message.frames.length)
      {
        return false;
      }
      var fakeFrame = null;
      for (var i = message.frames.length - 1; i >= 0; i--)
      {
        fakeFrame = {
          parent: fakeFrame,
          location: {
        ...
        var sitekey = message.frames[i].sitekey || null;
      ...
      var Policy = require("contentPolicy").Policy;
...
require.scopes["notification"] = (function()
{
  ...
  var Utils = require("utils").Utils;
  var _tempVar32 = require("matcher");
  var Matcher = _tempVar32.Matcher;
  var defaultMatcher = _tempVar32.defaultMatcher;
  var _tempVar33 = require("filterClasses");
  var Filter = _tempVar33.Filter;
  var RegExpFilter = _tempVar33.RegExpFilter;
  var WhitelistFilter = _tempVar33.WhitelistFilter;
  var INITIAL_DELAY = 1 * MILLIS_IN_MINUTE;
  var CHECK_INTERVAL = 1 * MILLIS_IN_HOUR;
  ...
  var Notification = exports.Notification = {
    ...
    _onDownloadSuccess: function(downloadable, responseText, errorCallback, redirectCallback)
    {
      try
      {
        ...
        for (var _loopIndex34 = 0; _loopIndex34 < data.notifications.length; ++_loopIndex34)
        {
          var notification = data.notifications[_loopIndex34];
      ...
      var _tempVar35 = downloader.processExpirationInterval(EXPIRATION_INTERVAL);
      Prefs.notificationdata.softExpiration = _tempVar35[0];
      Prefs.notificationdata.hardExpiration = _tempVar35[1];
    ...
    _getNextToShow: function(url)
    {
      ...
      var _tempVar36 = require("info");
      var addonName = _tempVar36.addonName;
      var addonVersion = _tempVar36.addonVersion;
      var application = _tempVar36.application;
      var applicationVersion = _tempVar36.applicationVersion;
      var platform = _tempVar36.platform;
      var platformVersion = _tempVar36.platformVersion;
      var notificationToShow = null;
      for (var _loopIndex37 = 0; _loopIndex37 < notifications.length; ++_loopIndex37)
      {
        var notification = notifications[_loopIndex37];
        ...
        if (typeof url === "string" || notification.urlFilters instanceof Array)
        {
          if (Prefs.enabled && typeof url === "string" && notification.urlFilters instanceof Array)
          {
            ...
            for (var _loopIndex38 = 0; _loopIndex38 < notification.urlFilters.length; ++_loopIndex38)
            {
              var urlFilter = notification.urlFilters[_loopIndex38];
        ...
        {
          var match = false;
          for (var _loopIndex39 = 0; _loopIndex39 < notification.targets.length; ++_loopIndex39)
          {
            var target = notification.targets[_loopIndex39];
    ...
    showNext: function(url)
    {
      ...
      if (notification)
      {
        for (var _loopIndex40 = 0; _loopIndex40 < showListeners.length; ++_loopIndex40)
        {
          var showListener = showListeners[_loopIndex40];
    ...
    getLocalizedTexts: function(notification, locale)
    {
      ...
      var localizedTexts = [];
      for (var _loopIndex41 = 0; _loopIndex41 < textKeys.length; ++_loopIndex41)
      {
        var key = textKeys[_loopIndex41];
    ...
    triggerQuestionListeners: function(id, approved)
    {
      ...
      for (var _loopIndex42 = 0; _loopIndex42 < listeners.length; ++_loopIndex42)
      {
        var listener = listeners[_loopIndex42];
        listener(approved);
      }
    },
...
require.scopes["antiadblockInit"] = (function()
{
  ...
  exports.initAntiAdblockNotification = function initAntiAdblockNotification()
  {
    ...
    {
      var urlFilters = [];
      for (var _loopIndex43 = 0; _loopIndex43 < subscription.filters.length; ++_loopIndex43)
      {
        var filter = subscription.filters[_loopIndex43];
        if (filter instanceof ActiveFilter)
        {
  ...
  window.URL = (function()
  {
    ...
    }))
    {
      ...
      URL = function(url, baseUrl)
      {
        ...
        anchor.href = url;
        for (var _loopIndex44 = 0; _loopIndex44 < URLProperties.length; ++_loopIndex44)
        {
          var prop = URLProperties[_loopIndex44];
...
require.scopes["whitelisting"] = (function()
{
  ...
  var getDecodedHostname = _tempVar45.getDecodedHostname;
  ...
  {
    var url = page.url;
    return defaultMatcher.whitelist.matchesAny(stringifyURL(url), RegExpFilter.typeMap.DOCUMENT, getDecodedHostname(url), false, null);
  };
  exports.isFrameWhitelisted = function(page, frame, typeMask)
  {
    while (frame)
    {
      ...
      var filter = defaultMatcher.whitelist.matchesAny(stringifyURL(url), typeMask || RegExpFilter.typeMap.DOCUMENT, documentHost, isThirdParty(url, documentHost), getKey(page, frame));
      if (filter)
      {
        return true;
      }
  ...
  exports.processKey = function(token, page, frame)
  {
    ...
    var _tempVar46 = token.split("_", 2);
    var key = _tempVar46[0];
    var signature = _tempVar46[1];
    key = key.replace(/=/g, "");
...
require.scopes["filterValidation"] = (function()
{
  ...
  var InvalidFilter = _tempVar47.InvalidFilter;
  var ElemHideBase = _tempVar47.ElemHideBase;
  ...
  exports.parseFilters = function(text)
  {
    ...
    for (var i = 0; i < lines.length; i++)
    {
      var _tempVar48 = parseFilter(lines[i]);
      var filter = _tempVar48.filter;
      var error = _tempVar48.error;
      if (filter)
      {
        filters.push(filter);
      }
...
{
  var exports = {};
  var _tempVar49 = require("url");
  var extractHostFromFrame = _tempVar49.extractHostFromFrame;
  var stringifyURL = _tempVar49.stringifyURL;
  var isThirdParty = _tempVar49.isThirdParty;
  var _tempVar50 = require("whitelisting");
  var getKey = _tempVar50.getKey;
  var isFrameWhitelisted = _tempVar50.isFrameWhitelisted;
  var defaultMatcher = require("matcher").defaultMatcher;
  var RegExpFilter = require("filterClasses").RegExpFilter;
  ...
  exports.composeFilters = function(details)
  {
    ...
    if (!isFrameWhitelisted(page, frame, RegExpFilter.typeMap.DOCUMENT))
    {
      ...
      for (var _loopIndex51 = 0; _loopIndex51 < details.urls.length; ++_loopIndex51)
      {
        var url = details.urls[_loopIndex51];
        var urlObj = new URL(url, details.baseURL);
        url = stringifyURL(urlObj);
        var filter = defaultMatcher.whitelist.matchesAny(url, typeMask, docDomain, isThirdParty(urlObj, docDomain), getKey(page, frame));
        if (!filter)
        {
      ...
      if (filters.length == 0 && !isFrameWhitelisted(page, frame, RegExpFilter.typeMap.ELEMHIDE))
      {
        ...
        for (var _loopIndex52 = 0; _loopIndex52 < selectors.length; ++_loopIndex52)
        {
          var selector = selectors[_loopIndex52];
...
{
  var exports = {};
  var _tempVar53 = require("icon");
  var startIconAnimation = _tempVar53.startIconAnimation;
  var stopIconAnimation = _tempVar53.stopIconAnimation;
  var Utils = require("utils").Utils;
  ...
  function openNotificationLinks()
  {
    ...
    {
      for (var _loopIndex54 = 0; _loopIndex54 < activeNotification.links.length; ++_loopIndex54)
      {
        var link = activeNotification.links[_loopIndex54];
