...
var updateNotifications = function(groups) {
    ...
    groupKeys.forEach(function(key) {
        ...
        var partyName = party ? party.name : text.get('me');
        ...
        if (desired.length == 1) {
            ...
            options.type = 'list';
            options.title = partyName || String.format(text.get('num_new_messages'), desired.length);
        ...
        if (party) {
            ...
            options.contextMessage = String.format(text.get('push_context_message'),
                                        party.name,
                                        new Date(Math.floor(firstPush.created * 1000)).toLocaleTimeString().replace(/:\d+ /, ' '));
        }
        ...
        options.onclick = function() {
            // Deal with incoming pushes from other people specially for Chrome
            if (!window.safari && firstPush.direction == 'incoming' && party && party.email_normalized) {
        ...
        var toThisDevice = firstPush.target_device_iden == pb.local.device.iden;
